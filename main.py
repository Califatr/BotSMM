...
#TODO удалить себя отсюда
admin_ids = [205423286, 127362323]
customer_ids = []
contractor_ids = []
accountant_ids = []
...
async def create_new_customer(vk_id):
    ...
    database_request("INSERT INTO public.customer(customer_surname, customer_name, customer_vk_id) VALUES ('{0}', '{1}', '{2}');".format(surname, name, vk_id), False)
...
async def create_new_order(vk_id, oblast, price, description):
    ...
    customer_id = database_request("SELECT customer_id FROM public.customer WHERE customer_vk_id LIKE '{0}'".format(vk_id))
    customer_id = customer_id[0][0]

    order_id = database_request("INSERT INTO public.order (customer_id, order_desiredprice, order_subjectarea, order_requirements) VALUES ('{0}', '{1}', '{2}', '{3}');SELECT LASTVAL();".format(customer_id, price, oblast, description), True)
    order_id = order_id[0][0]
    database_request(f"INSERT INTO public.account(order_id, account_login, account_password, account_socialnetwork) VALUES ('{order_id}', 'отсутствует', 'отсутствует', 'отсутствует');", False)
...
async def get_order_data(order_id):
    result = database_request(f"SELECT order_desiredprice, order_subjectarea, order_requirements FROM public.order WHERE order_id={order_id}")
...
async def get_order_data_detailed(order_id):
    result = database_request(f"SELECT order_desiredprice, order_subjectarea, order_requirements, account_socialnetwork, account_login, account_password FROM public.order, public.account WHERE public.account.order_id={order_id} AND public.order.order_id={order_id}")
    result = result[0]
...
async def add_new_content_to_order(order_id, text):
    database_request(f"INSERT INTO public.content(order_id, content_text) VALUES ('{order_id}', '{text}');", False)
    contractor_id = database_request(f"SELECT contractor_vk_id FROM public.contractor WHERE contractor_id IN (SELECT contractor_id FROM public.order WHERE order_id={order_id})")
...
async def contractor_accept_order(vk_id, order_id):
    contractor_id = database_request(f"SELECT contractor_id, contractor_name FROM public.contractor WHERE contractor_vk_id LIKE '{vk_id}'")
    contractor_name = contractor_id[0][1]
    contractor_id = contractor_id[0][0]
    user_vk_id = database_request(f"SELECT customer_vk_id FROM public.customer WHERE customer_id IN (SELECT customer_id FROM public.order WHERE order_id={order_id})")
    user_vk_id = user_vk_id[0][0]
    database_request(f"UPDATE public.order SET contractor_id='{contractor_id}' WHERE order_id='{order_id}';", False)
...
async def create_new_contractor(vk_id):
    ...
    name = user_info[0].first_name
    surname = user_info[0].last_name
    database_request("INSERT INTO public.contractor(contractor_surname, contractor_name, contractor_vk_id) VALUES ('{0}', '{1}', '{2}');".format(surname, name, vk_id), False)
...
async def create_new_accountant(vk_id):
    ...
    name = user_info[0].first_name
    surname = user_info[0].last_name
    database_request("INSERT INTO public.accountant(accountant_surname, accountant_name, accountant_vk_id) VALUES ('{0}', '{1}', '{2}');".format(surname, name, vk_id), False)
    accountant_ids.append(vk_id)

async def update_account_info(order_id, social, login, password):

    database_request(f"UPDATE public.account SET account_login='{login}', account_password='{password}', account_socialnetwork='{social}' WHERE order_id={order_id};", False)

async def create_new_report(order_id, contractor_vk_id, scope, numberpublications):
    contractor_id = database_request(f"SELECT contractor_id FROM public.contractor WHERE contractor_vk_id LIKE '{contractor_vk_id}'")[0][0]
    database_request(f"INSERT INTO public.report(order_id, contractor_id, report_startdate, report_scope, report_numberpublications) VALUES ({order_id}, {contractor_id}, CURRENT_DATE, '{scope}', '{numberpublications}');", False)

    user_vk_id = database_request(f"SELECT customer_vk_id FROM public.customer WHERE customer_id IN (SELECT customer_id FROM public.order WHERE order_id={order_id})")[0][0]
...
async def create_new_profit(accountant_id, order_id, initialcost, costmaintenance, totalcost):
    database_request(f"INSERT INTO public.profit(accountant_id, order_id, profit_initialcost, profit_costmaintenance, profit_totalcost) VALUES ('{accountant_id}', '{order_id}', '{initialcost}', '{costmaintenance}', '{totalcost}');", False)
...
def default_state(user_data, message:Message):
    ...
    if message.from_id in contractor_ids:
        my_keyboard.row()
        my_keyboard.add(Text("Доступные Заказы"), color=KeyboardButtonColor.POSITIVE)
        my_keyboard.row()
    ...
    if message.from_id in admin_ids:
        my_keyboard.row()
        my_keyboard.add(Text("Добавить Исполнителя"), color=KeyboardButtonColor.POSITIVE)
        my_keyboard.add(Text("Добавить Бухгалтера"), color=KeyboardButtonColor.POSITIVE)
...
def new_content_text(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Отправить контент"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
    return {
        "text":final_answer,
...
def new_order_message_text(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Отправить Сообщение Заказчику"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.row()
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
    return {
...
def new_contractor_vk_id(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Добавить Нового Исполнителя"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.row()
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
    return {
...
def new_accountant_vk_id(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Добавить Нового Бухгалтера"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.row()
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
    return {
...
def update_account_text(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Обновить Аккаунт!"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
    return {
        "text":final_answer,
...
def new_report_numberpublications(user_data, message:Message):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text("Отправить Отчёт!"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
...
async def my_orders(message: Message):
    my_orders = database_request(f"SELECT order_id, order_subjectarea FROM public.order WHERE customer_id IN (SELECT customer_id FROM public.customer WHERE customer_vk_id='{message.from_id}')")
    my_keyboard = Keyboard(one_time=True, inline=False)
...
async def new_content(message: Message):
    my_orders = database_request(f"SELECT order_id FROM public.order WHERE customer_id IN (SELECT customer_id FROM public.customer WHERE customer_vk_id='{message.from_id}')")
    my_keyboard = Keyboard(one_time=True, inline=False)
    for element in my_orders:
        my_keyboard.row()
...
async def available_orders(message:Message):
    orders = database_request(f"SELECT order_id, order_desiredprice, order_subjectarea, order_requirements FROM public.order WHERE contractor_id ISNULL")
    my_keyboard = Keyboard(one_time=True, inline=False)
    for element in orders:
        my_keyboard.row()
        my_keyboard.add(Text(f"!доступный_заказ {element[0]}"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.row()
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
...
async def my_order(message: Message, args: Tuple[str]):
    ...
    my_keyboard = Keyboard(one_time=True, inline=False)
    my_keyboard.add(Text(f"!принять_заказ {args[0]}"), color=KeyboardButtonColor.POSITIVE)
    my_keyboard.row()
    my_keyboard.add(Text("Отмена"), color=KeyboardButtonColor.NEGATIVE)
    my_keyboard = my_keyboard.get_json()
...
async def accepted_orders(message: Message):
    my_orders = database_request(f"SELECT order_id FROM public.order WHERE contractor_id IN (SELECT contractor_id FROM public.contractor WHERE contractor_vk_id LIKE '{message.from_id}')")
    my_keyboard = Keyboard(one_time=True, inline=False)
    for element in my_orders:
        my_keyboard.row()
...
async def send_profit(message:Message):
    my_orders = database_request(f"SELECT order_id FROM public.order WHERE order_id NOT IN (SELECT order_id FROM public.profit)")
    my_keyboard = Keyboard(one_time=True, inline=False)
    for element in my_orders:
        my_keyboard.row()
...
async def send_profit(message:Message):
    ...
    accountant_id = database_request(f"SELECT accountant_id FROM public.accountant WHERE accountant_vk_id='{vk_id}'")[0][0]
...
async def get_profits_id(message: Message, args: Tuple[str]):
    ...
    result = database_request(f"SELECT * FROM public.report WHERE order_id={order_id} ORDER BY report_startdate ASC")
    for element in result:
...
async def my_profits(message:Message):
    ...
    result = database_request(f"SELECT order_id, profit_totalcost FROM public.profit WHERE order_id IN {my_orders_string}")
